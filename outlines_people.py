...
def download_images(json_path, images_dir):
    """
    Reads a JSON array of objects with an "imageUrl" field,
    ...
    for i, item in enumerate(data):
        ...
        if not url:
            print(f"[!] No URL for item {i}, skipping.")
            continue
        try:
...
def create_outlines(masks_dir, outlines_dir, low_thresh=50, high_thresh=150):
    ...
    for mask_path in sorted(masks_dir.glob("*.png")):
        ...
        rgba[..., :3] = 255           # white line
        rgba[..., 3] = edges          # edge map → alpha
...
def stack_outlines(outlines_dir, output_path, canvas_size=None):
    ...
    # determine canvas size from first outline (or use provided)
...
def main():
    # adjust these if your JSON lives elsewhere
    json_path = Path("plusimg.json")
